...
![newtons 2nd law](.Fenora%20images/2.jpg)
...
Think about an arbitrary, infinitesimal displacement <!-- \delta\vec{r}δr--> of the position of our particle under the net force. We can say <!-- \vec{F}\cdot \delta\vec{r} = m\vec{a}\cdot\delta\vec{r}F⋅δr=ma⋅δr --> . If we integrate along a finite displacement, we find that
<!-- W = \int \vec{F}_\text{net} \cdot \delta\vec{r}W=∫Fnet​⋅δr -->

and therefore

<!-- \int \left(\vec{F}_\text{net}-m\vec{a}\right)\cdot \delta\vec{r} = 0.∫(Fnet​−ma)⋅δr=0 -->.
...
<!-- F_i = -\frac{\partial V(r_1, \ldots, r_n)}{\partial r_i}.Fi​=−∂ri​∂V(r1​,…,rn​)​.-->
...
First, we rewrite Newton's second law <!-- \vec{F} = m\ddot{\vec{r}}F=mr¨--> as
...
<!-- \vec{F}_\text{net} \cdot \delta\vec{r} = m\frac{d^2\vec{r}}{dt^2} \cdot \delta\vec{r},Fnet​⋅δr=mdt2d2r​⋅δr, -->
where <!-- \delta \vec{r}δr --> is some infinitesimal movement of the particle. Immediately, we can rewrite the right-hand side as
<!-- m\left[ \frac{d}{dt}\left( \frac{d\vec{r}}{dt}\cdot \delta\vec{r} \right) - \frac{d\vec{r}}{dt}\cdot\frac{d\delta\vec{r}}{dt}\right],m[dtd​(dtdr​⋅δr)−dtdr​⋅dtdδr​], -->

where we have applied the identity <!-- \left(xy\right)^\prime = x^\prime y + y^\prime x(xy)′=x′y+y′x. -->

Above, we have an equation for the vector <!-- \vec{r}r --> that doesn’t reference a particular choice of coordinates, no Cartesian or spherical, etc.. We now expand the RHS in a coordinate basis <!-- r_1, \ldots, r_n:r1​,…,rn​ --> :

<!-- \begin{aligned} &m\sum_k\left[\frac{d}{dt}\left( \frac{d\vec{r}}{dt}\cdot \frac{\partial \vec{r}}{\partial r_k}\delta r_k \right) - \frac{d\vec{r}}{dt}\cdot\frac{\partial\dot{\vec{r}}}{\partial r_k} \delta r_k\right] \\ =& m\sum_k\left[\frac{d}{dt}\left( \dot{\vec{r}}\cdot \frac{\partial \vec{r}}{\partial r_k}\right) - \dot{\vec{r}}\cdot\frac{\partial\dot{\vec{r}}}{\partial r_k} \right]\delta r_k. \end{aligned}=​mk∑​[dtd​(dtdr​⋅∂rk​∂r​δrk​)−dtdr​⋅∂rk​∂r˙​δrk​]mk∑​[dtd​(r˙⋅∂rk​∂r​)−r˙⋅∂rk​∂r˙​]δrk​.​ -->#

Now, for any <!-- \vec{r}r -->, we have the following identity, known as dot-cancellation: 


## _Dot-cancellation_ theorem

_If we expand a vector <!-- \vec{r}r --> in any coordinate basis <!-- \{r_1,\ldots,r_n\}{r1​,…,rn​} -->, we have_
<!-- \frac{\partial \vec{r}}{\partial r_k} = \frac{\partial \dot{\vec{r}}}{\partial \dot{r}_k}.∂rk​∂r​=∂r˙k​∂r˙​ -->.
...
<!-- m\sum_k\left[\frac{d}{dt}\left( \dot{\vec{r}}\cdot \frac{\partial \dot{\vec{r}}}{\partial \dot{r}_k}\right) - \dot{\vec{r}}\cdot\frac{\partial\dot{\vec{r}}}{\partial r_k} \right]\delta r_k.mk∑​[dtd​(r˙⋅∂r˙k​∂r˙​)−r˙⋅∂rk​∂r˙​]δrk​. -->

Recognizing <!-- \dot{\vec{r}} = \vec{v}r˙=v --> , we rewrite this as
...
where in the second line we've made the replacement T = <!-- \frac12 mv^2.T=21​mv2. -->
...
Now, remember that the small displacement <!-- \delta \vec{r}δr was arbitrary. In three dimensions, we could have \delta\vec{r} = \langle \delta_x, 0, \delta_z \rangleδr=⟨δx​,0,δz​⟩ or \langle 0, 0, \delta_z \rangle⟨0,0,δz​⟩-->. But the equation must be true independent of the displacement, i.e. every term in the sum is equal to zero independent of the coordinate displacements <!-- \delta r_k.δrk​-->. Therefore, we have
<!-- 0 = \frac{d}{dt}\left(\frac{\partial T}{\partial \dot{r}_k}\right) + \frac{\partial\left(V - T\right)}{\partial r_k}0=dtd​(∂r˙k​∂T​)+∂rk​∂(V−T)​ -->
for each _k_.
For conservative forces, _V_ is a pure function of the position variables so that <!--\frac{\partial V}{\partial \dot{r}_k}=0∂r˙k​∂V​=0-->. We can therefore move _V_ into the derivative and write

<!--\frac{\partial\left(T - V\right)}{\partial r_k} = \frac{d}{dt}\frac{\partial \left(T-V\right)}{\partial \dot{r}_k}.∂rk​∂(T−V)​=dtd​∂r˙k​∂(T−V)​-->.
...
<!--\frac{\partial L}{\partial r_k} = \frac{d}{dt}\frac{\partial L}{\partial \dot{r}_k} .∂rk​∂L​=dtd​∂r˙k​∂L​-->.
...
<!-- L(r, \dot{r}) \equiv T(r, \dot{r}) - V(r).L(r,r˙)≡T(r,r˙)−V(r)-->.
...
Because the mass slips down the inclined plane, we can choose our coordinate to be the distance along the plane _s_. In this coordinate system, the kinetic energy of the mass is given by <!--T=\frac12 m \dot{s}^2T=21​ms˙2-->. If we define the initial potential energy to be zero, then the potential energy as a function of _s_ is given by <!--V=+mgs\sin\thetaV=+mgssinθ-->, where _θ_ is the angle of the incline. 

Thus our Lagrangian is given by

<!--L = \frac12 m \dot{s}^2 - mgs\sin\theta.L=21​ms˙2−mgssinθ-->.

As _s_ is the only variable coordinate in this problem, we have one Euler equation to consider:

<!--\begin{aligned} \frac{d}{dt}\frac{\partial L}{\partial \dot{s}} &= \frac{\partial L}{\partial s} \\ m\frac{d}{dt}\dot{s} &= - mg\sin\theta \\ m\ddot{s} &= - mg\sin\theta\\ \ddot{s} &= - g\sin\theta, \end{aligned}dtd​∂s˙∂L​mdtd​s˙ms¨s¨​=∂s∂L​=−mgsinθ=−mgsinθ=−gsinθ-->,​
...
Imagine we have a star of mass _M_ with a much greater mass than our planet of mass _m_. The planet and star interact through the gravitational potential <!--V(r) = -GMm/r,V(r)=−GMm/r-->, where _r_ is the distance separating the star and planet. What are the equations of motion?
...
The kinetic energy of the planet is given by <!--T = \frac12 m \dot{r}^2 + \frac12 m \left(r\dot{\theta}\right)^2T=21​mr˙2+21​m(rθ˙)2-->.

Thus our Lagrangian is given by

<!--L = \frac12 m \dot{r}^2 + \frac12 m \left(r\dot{\theta}\right)^2 + GMm/r.L=21​mr˙2+21​m(rθ˙)2+GMm/r-->.

Both _r_ and _θ_ can vary, so we have two Euler equations to set up. The one for _r_ is given by

<!--\begin{aligned} \frac{d}{dt}\frac{\partial L}{\partial \dot{r}} &= \frac{\partial L}{\partial r} \\ m\ddot{r} &= mr\dot{\theta}^2 - GMm/r^2, \end{aligned}dtd​∂r˙∂L​mr¨​=∂r∂L​=mrθ˙2−GMm/r2-->,​

which is the equation of motion we expect for _r_.

For _θ_ we have

<!--\begin{aligned} \frac{d}{dt}\frac{\partial L}{\partial \dot{\theta}} &= \frac{\partial L}{\partial \theta} \\ \frac{d}{dt}\left(mr^2\dot{\theta}\right) &= 0. \end{aligned}dtd​∂θ˙∂L​dtd​(mr2θ˙)​=∂θ∂L​=0-->.​
...
Kinetic energy, in Cartesian coordinates, is given by <!--T = \frac12 m \dot{r}^2T=21​mr˙2-->. 

Since potentials are functions of position, the velocity <!--\dot{r}r˙--> will only ever appear in the kinetic energy term. Thus, the quantity <!--\frac{\partial L}{\partial \dot{r}}∂r˙∂L​ in the Euler equation will always be equal to m\dot{r}mr˙-->. However, this is just the momentum of the particle. Thus, we can say that <!--p_r = \frac{\partial L}{\partial \dot{r}}pr​=∂r˙∂L​-->, i.e. the momentum of the particle is just the derivative of the Lagrangian with respect to the velocity.

Thus, we can rewrite the Euler equation as

<!--\frac{d}{dt}p_r = \frac{\partial V}{\partial r}.dtd​pr​=∂r∂V​-->.

Notice also, in the orbital mechanics example, that the second Euler equation produced <!--\frac{\partial L}{\partial \dot{\theta}} = mr^2\dot{\theta}∂θ˙∂L​=mr2θ˙-->, the angular momentum of the particle. Thus, we have <!--p_\theta = \frac{\partial L}{\partial \dot{\theta}}pθ​=∂θ˙∂L​-->, just as we had for linear momentum above.
...
<!--\dot{p}_i = \frac{\partial L}{\partial r_i}.p˙​i​=∂ri​∂L​.-->
...
If the coordinate r_krk​ does not appear in the Lagrangian of a system, then the corresponding momenta is a conserved quantity.

Suppose the coordinate r_krk​ makes no appearances in the Lagrangian, then

<!--\dot{p}_k = \frac{\partial L}{\partial r_k} = 0.p˙​k​=∂rk​∂L​=0-->.
